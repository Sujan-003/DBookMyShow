...
app.get('/api/movies', async (req, res) => {
  try {
    ...
    if (search) {
      query = 'SELECT * FROM movies WHERE LOWER(title) LIKE $1';
...
app.get('/api/shows/:showId', async (req, res) => {
  ...
  try {
    ...
    const bookingsResult = await pool.query('SELECT seats FROM bookings WHERE show_id = $1', [showId]);
...
app.post('/api/bookings', async (req, res) => {
  ...
  // Basic validation
  if (!showId || !seats || !Array.isArray(seats) || seats.length === 0 || totalAmount === undefined) {
    return res.status(400).json({ error: 'Invalid booking data provided' });
  }

  try {
    // Insert the new booking into the bookings table
    // Assuming 'seats' column is TEXT[]
    const result = await pool.query(
      'INSERT INTO bookings (show_id, seats, total_amount) VALUES ($1, $2, $3) RETURNING booking_id',
      [showId, seats, totalAmount]
    );
...
app.get('/api/bookings/:bookingId', async (req, res) => {
  ...
  try {
    ...
    const responseData = {
      ...
      seats: bookingData.seats, // Assuming seats is already an array
      total_amount: bookingData.total_amount,
      created_at: bookingData.created_at, // Assuming a created_at column in bookings table
      show: {
        show_id: bookingData.show_id,
